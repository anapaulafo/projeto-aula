...
const API_KEY = "bf5852ad3d3c8aeef949e17138b85599"; 
  ...
  try {
    const results = await Promise.all(
      movieIds.map(async (id) => {
        ...
        if (!resApi.ok) {
          console.error(
            `Erro ao buscar filme ID ${id}: ${resApi.status} ${resApi.statusText}`
          );
          return null;
        }
    ...
    console.error("Erro fatal ao processar requisições:", error);
